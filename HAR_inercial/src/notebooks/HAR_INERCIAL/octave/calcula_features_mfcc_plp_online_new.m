% MAIN FUNCTION
function [OUTPUT] = calcula_features_mfcc_plp_online_new(AccXorig,AccYorig,AccZorig, win_size, overlap, debug)
...
OUTPUT = [fmfcc3D(AccX',AccY',AccZ', win_size, step)];  %30 features
#OUTPUT = [OUTPUT mean(AccX) mean(AccY) mean(AccZ) std(AccX) std(AccY) std(AccZ)];  %6 features
# FUNCTIONALS = [mean(AccX) mean(AccY) mean(AccZ) std(AccX) std(AccY) std(AccZ)]
...
output_file = '/home/aux_features.arff';
