...
from AR_modules.config.general import RASPI, WEKA, debug, TRAIN, TRAIN_FROM_DATA_SET, TRAINING_LENGTH_PER_ACTIVITY
...
def Recognize_lstm(model, tensor):
    ...
    if RASPI:
        ...
        elif MODELS_PREFIX == "DRIVE_BACKHAND_SERVE_LOB":
            if int(pred) == 0:
...
if __name__ == "__main__":
    try:
        ...
        if TRAIN:
            raw_data_filename = RAW_DATA_PATH + "raspi_raw_data_online_" + MODELS_PREFIX + ".csv"
            fd_raw_data = open(raw_data_filename, "a")
            TrainNewModel(TRAINING_LENGTH_PER_ACTIVITY)
        
        if TRAIN_FROM_DATA_SET:
            TrainNewModelFromDataset()
